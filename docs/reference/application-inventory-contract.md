...
Pada section Sources, `Enter` membuka halaman Application Inventory yang dibatasi pada source terpilih, `i` membuka/menutup detail source, dan `c` memeriksa semua source satu per satu. Pada section Application Inventory, `Enter` membuka/menutup detail record, `u` menjalankan upgrade seluruh aplikasi pada source aktif, dan `s` atau `Esc` kembali ke Sources. Panel PATH hanya ditampilkan di Sources. Horizontal scroll Inventory menggunakan `←`/`→` atau `h`/`l`.

Pemeriksaan source melalui `c` dimulai dengan kolom jumlah aplikasi dan jumlah update dalam keadaan kosong. Hanya source berstatus `AVAILABLE` yang dimasukkan ke antrean pemeriksaan; source `DISABLED` dan `ERROR` dilewati tanpa command. Collector dijalankan serial sesuai urutan baris Sources, dan `total` progres hanya menghitung source `AVAILABLE`. Setelah satu source selesai, kedua hasilnya langsung diperbarui pada baris source tersebut. Panel Detail menampilkan `Checking source N/total` dan nama source aktif selama proses.
